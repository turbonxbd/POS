...
| **Super Admin** (`superadmin.html`) | Platform operator only, fully separated from every merchant surface: plans & pricing, payment methods, payment-request approvals (incl. **bulk approve**), merchant list (paginated, tag filter, **internal notes & tags**, **message a merchant**, **reset owner password**), subscriptions, account lock/unlock, revenue, an **activity log** of every operator action, automatic **backups**, support, live chat. |
...
| **Cashier POS** | Barcode + search + category grid, cart with per-line & cart discounts (incl. **product / category-scoped** discounts & coupons), price override, hold/resume, customer create/select, cash/card/mobile/**mixed** payment with change, **charge-to-account (credit) sales**, **loyalty-point redemption**, atomic checkout, receipt print. Keyboard-first (F1/F2/F4/F8/F9). An offline-queue indicator shows when sales are waiting to sync. |
...
| **Purchasing** | Purchase orders with **per-line discount / VAT and a freight charge**, **edit while draft or ordered**, partial receiving (increases stock), purchase returns (decreases stock), supplier statements & payments. |
| **Cash register** | Open with float, cash in/out, **blind or open close**, expected-vs-counted reconciliation, printable **X-report (mid-shift) and Z-report**, admin force-close. |
| **Reports** | ~23 reports (sales, profit, inventory, **reorder / low-stock**, **dead-stock / ageing**, stock movement, tax/VAT, cashier, product/category performance, daily closing, returns & exchanges, **loyalty points**, …) with date + **branch (incl. All branches)** filters, print, CSV/JSON export. |
